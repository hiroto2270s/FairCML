...
def hr_ndcg(all_scores, ground_truth_list, k):
    """
    全ユーザーのスコア行列に対して、HR@K と NDCG@K を計算する。

    Args:
        all_scores (np.ndarray): 全テストユーザー×全アイテムのスコア行列。
        ground_truth_list (list of list): 各テストユーザーの正解アイテムリスト。
        k (int): 評価のためのトップK。

    Returns:
        (float, float): (平均HR, 平均NDCG)
    """
    hrs = []
    ndcgs = []

    for i, scores in enumerate(all_scores):
        rank_list = np.argsort(-scores)[:k]
        ground_truth = ground_truth_list[i]

        if not ground_truth:
            continue

        hits = [item for item in rank_list if item in ground_truth]
        ...
        ranks = [np.where(rank_list == item)[0][0] + 1 for item in hits]
        
        dcg = sum([1.0 / np.log2(rank + 1) for rank in ranks])

        idg_len = min(len(ground_truth), k)
        idcg = sum([1.0 / np.log2(i + 2) for i in range(idg_len)])
        
        ndcg = dcg / idcg if idcg > 0 else 0.0
        ndcgs.append(ndcg)

    return np.mean(hrs) if hrs else 0.0, np.mean(ndcgs) if ndcgs else 0.0

#配列aryの中から上位n個のインデックスを取得する関数
...
#モデルの評価指標(AUCm,acc,f1_micro,f1_macro)を計算する関数
def metrics(preds_list,labels_list,probs_list,dataset_len,correct):
    ...
    cat_preds_list = torch.cat(preds_list,0).data.to(device).numpy()
    cat_labels_list = torch.cat(labels_list,0).data.to(device).numpy()
    cat_probs_list = torch.cat(probs_list,0).data.to(device).numpy()
    
    AUC = 0
    n_classes = 6
    cat_labels_list_bin = label_binarize(cat_labels_list, classes=list(range(n_classes)))
    AUC = roc_auc_score(cat_labels_list_bin, cat_probs_list,average="macro", multi_class="ovr")
    AUC = max(AUC, 1 - AUC)
...
#性別に基づくモデルの評価指標(AUCm,acc,f1_micro,f1_macro)を計算する関数
def metrics_gender(preds_list,labels_list,probs_list,dataset_len,correct):
    ...
    AUC = adjusted_auc(cat_labels_list, cat_probs_list)
    # fpr, tpr, thresholds = roc_curve(cat_labels_list, cat_probs_list, pos_label=1)
    # AUC = auc(fpr, tpr)
    acc = 100. * correct / dataset_len
...
def fairness_K_re2(user_embedding,item_embedding,topk,test_user_list,all_user_list,gender_data):
    ...
    '''
    user_e = torch.tensor(user_embedding).to(device).detach().numpy()
    item_e = torch.tensor(item_embedding).to(device).detach().numpy()
...
def calculate_fairness_CML(score_matrix, user_labels, train_user_list, top_k, is_distance=False):
    """
    CMLモデルのスコア行列に基づき、複数のユーザーグループ（user_labels）に対する
    Provider Fairness (Fairness@K と Fairness@All) を計算する。
    スコアのスケールを揃えるため、正規化を行う。

    Args:
        score_matrix (torch.Tensor): ユーザー×アイテムのスコア行列 (user_size, item_size)。
        user_labels (list or np.ndarray): 各ユーザーの属性ラベル（例: 0-5）。
        train_user_list (list of dict): 各ユーザーが訓練データで評価したアイテムの辞書。
        top_k (int): Fairness@Kを計算するためのK値。
        is_distance (bool): スコアが行列分解の評価値ではなく、距離（低いほど良い）かどうか。
                            Trueの場合、スコアを反転させて正規化する。

    Returns:
        tuple: (fairness_all, fairness_k)
            - fairness_all: 全ての未評価アイテムを対象とした公平性スコア。
            - fairness_k: トップK推薦アイテムを対象とした公平性スコア。
    """
    begin_time = time.time()
    user_size, item_size = score_matrix.shape

    score_matrix_np = score_matrix.cpu().numpy()

    # is_distanceがTrueの場合、スコアは距離なので、小さいほど良い。
    # スコアの方向性を「大きいほど良い」に統一するため、符号を反転する。
    if is_distance:
        score_matrix_np = -score_matrix_np

    # スコアを0-1の範囲に正規化して、異なるモデル間でのスケールの違いを吸収する
    min_val = np.min(score_matrix_np)
    max_val = np.max(score_matrix_np)
    if max_val > min_val:
        score_matrix_np = (score_matrix_np - min_val) / (max_val - min_val)


    labels = np.array(user_labels)
    unique_labels = sorted(np.unique(labels))

    # 訓練データに含まれるアイテムを推薦から除外するためにマスクします。
    # score_matrix_np のコピーを作成して、元のスコア行列を変更しないようにします。
    masked_score_matrix = score_matrix_np.copy()
    for u in range(user_size):
        # train_user_list[u] はユーザーuが訓練データで評価したアイテムの辞書です。
        if u < len(train_user_list) and train_user_list[u]:
            train_items = list(train_user_list[u].keys())
            masked_score_matrix[u, train_items] = -np.inf  # スコアを非常に低い値に設定してマスクします。

    # 各ユーザーについてトップKのアイテムインデックスを取得します。
    # argpartition は高速ですが、トップK内の順序は保証されません。
    # ここでは、スコアに基づいてソートされたトップKリストを取得するために argsort を使用します。
    top_k_indices = np.argsort(-masked_score_matrix, axis=1)[:, :top_k]

    # --- 2. 各ユーザーグループに対する公平性スコアの計算 ---
    # 各グループの平均スコアを格納するリスト
    group_mean_scores_all = []
    group_mean_scores_k = []

    # 各ユーザーグループのインデックスを取得
    user_groups = {}
    for label in unique_labels:
        user_groups[label] = np.where(labels == label)[0]

    # 各ユーザーグループに対してループ
    for label, group in user_groups.items():
        if len(group) == 0:
            continue

        # グループ内のユーザーのスコア行列を取得
        group_score_matrix = score_matrix_np[group]
        group_masked_score_matrix = masked_score_matrix[group]

        # Fairness@All: 全ての未評価アイテムを対象としたグループの平均スコア
        group_mean_scores_all.append(np.mean(group_score_matrix))

        # Fairness@K: トップK推薦アイテムを対象としたグループの平均スコア
        top_k_scores = np.take_along_axis(group_masked_score_matrix, top_k_indices[group], axis=1)
        group_mean_scores_k.append(np.mean(top_k_scores))

    # --- 3. グループ間の公平性指標を計算 ---
    # グループ平均スコアの最大値と最小値の差を計算
    fairness_all = 0
    if group_mean_scores_all:
        fairness_all = np.max(group_mean_scores_all) - np.min(group_mean_scores_all)

    fairness_k = 0
    if group_mean_scores_k:
        fairness_k = np.max(group_mean_scores_k) - np.min(group_mean_scores_k)


    end_time = time.time()
    elapsed_time = end_time - begin_time
    print(f"Fairness calculation completed in {elapsed_time:.2f} seconds.")

    return fairness_all, fairness_k

#NIPS2017で提案された公平性指標(U値)を計算する関数
#U値は、男女の予測値と実際の評価値の差を計算し、その平均を取ることで求められる
def fairness_nips2017_1(test_pair,users_embedding,items_embedding,gender_data,item_size):
    ...
    user_e = torch.tensor(users_embedding).to(device).detach().numpy()
    item_e = torch.tensor(items_embedding).to(device).detach().numpy()
...
# NIPS2017で提案された公平性指標(U値)を特定のアイテムに限定して計算する関数
# U値は、男女の予測値と実際の評価値の差を計算し、その平均を取ることで求められる
...
#新しい公平性指標(U値)を計算する関数
#性別ごとに予測スコアと実際の評価値の絶対誤差を比較し、公平性を評価
...
#MovieLens 1M データセットを読み込むためのクラス
class MovieLens1M():
...
# --------------------------load feature data-------------------------#
#特定のジャンル（例: Romance, Action）のアイテムIDを取得します。
def load_item_kind():
